...
class EditSetMain extends StatefulWidget {
  const EditSetMain({Key? key, required this.document, required this.docID}) : super(key: key);
  final Map<String, dynamic> document;
  final String docID;

  @override
  _EditSetMainState createState() => _EditSetMainState();
}

final String? uid = AuthHelper().getCurrentUID();

class _EditSetMainState extends State<EditSetMain> {
  @override
  void initState() {
    super.initState();
    _titleController = TextEditingController(text: widget.document["setName"]);
    type = widget.document["type"].toString();
    deadline = (widget.document["date"] as Timestamp).toDate();
    setPathSub = widget.docID;
    _addDataToList();
  }
  ...
  _addDataToList () {
    FirebaseFirestore.instance.collection("users").doc(uid).collection("Tasks").where('setPath', isEqualTo: setPathSub).orderBy("date").get()
            ...
            for (var doc in querySnapshot.docs) {
              subTasks.add(
                  SubTasksEditing(
                      ...
                      setPath: doc["setPath"].toString(), id: doc.id
                  ),
              );
            }
          });
